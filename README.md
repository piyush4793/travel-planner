...
The landing picker carries a **"When are you going?"** month row and a **region** row (both fold in what were the Calendar and Discover tabs). Selecting a month re-ranks the whole board by seasonality via `monthFit`/`rankByMonthFit` (`src/core/utils/monthFit.ts`), reading `bestMonths`/`worstMonths` baked into `data/rules/index.json` so it works offline over the full 198 with no lazy rule loads. Budget tiering (used in cost badges) buckets a `₹`-range string by its **midpoint** using the active budget basis: ₹ Budget (< ₹1.5L) / ₹₹ Mid (₹1.5L–₹3L) / ₹₹₹ Premium (₹3L+).
