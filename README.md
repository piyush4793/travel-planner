...
- Calendar rows come from Recents; Cinematic uses the active Plan route as its map backdrop
- Discover is pure browse → Plan: region/search browse plus a multi-destination tray that starts the Plan wizard

---

### Filters (Calendar and Discover have their own)
The **Calendar** view has its own Month filter over Recents; **Discover** has region browse and search. Budget tiering (used in Discover and cost badges) buckets a `₹`-range string by its **midpoint** using the active budget basis: ₹ Budget (< ₹1.5L) / ₹₹ Mid (₹1.5L–₹3L) / ₹₹₹ Premium (₹3L+).
...
The old destination drawer and per-country edit form were retired in favor of the Plan-first flow. Start planning from the Plan landing picker, Discover's multi-select tray, Calendar rows, or a saved trip; all itinerary shaping now happens inside the Plan wizard's Places step and Route Canvas. Per-country manual editing and panel-based AI-plan browse/compare are not currently available. Travel style was removed for now and may return later.
...
- Total statement coverage is ~90% (1160 tests). The `ai` folder, shared Plan/itinerary components, the Trips/Discover/Calendar views (incl. trip intake flows), and the platform-backup stack (`core/platform/*`, `core/adapters/backup/*`, `StorageLocationCard`) are now covered. `importParser`, `useChatSession`, `core/storage`, and `App.tsx` orchestration are unit/integration-covered.
