...
- **Share** — the panel Share action opens the phone's **native share sheet with the itinerary PDF attached** (generated in-browser — no download step). The shared PDF is now **pixel-identical to Export**: it rasterises the very same itinerary document the **Export PDF** prints, so you get the same emerald/ivory theme, serif display titles, **Roamwise brand logo** letterhead, **real country flags**, crisp stat icons, **per-country "Stop N" sections** (day range · cost · best months · that country's own practical notes) and the footer **"Plan your own trip" link** — scaling to any number of stops. Pages **break at card boundaries**, so a day or section never gets split across a page. (Because the shared PDF is image-based for perfect visual parity, its text isn't selectable/searchable — the **Export PDF** via the browser print dialog keeps selectable text.) It falls back to a native text share where file sharing isn't supported, and to copying a rich text summary + app link on desktop
