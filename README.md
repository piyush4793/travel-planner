...
Vite 5 + React 18 + TypeScript + Tailwind CSS + MapLibre GL JS. Zero runtime dependencies beyond React + MapLibre. No routing library, no state management library. Static site — deploy to Netlify/Vercel free tier. Rule-based itinerary content is stored in lazy-loaded per-country JSON files under `data/rules/`, with each destination carrying transport legs, SIM/app tips, hotel/meal picks, day-trip guidance where needed, and city image search terms so research-heavy updates like rail-first Laos journeys, Cambodia's Angkor-to-Koh-Rong route, mixed city-and-safari South Africa routes, Nepal's heritage-and-safari circuit, rail-heavy UK heritage circuits, China's VPN-aware rail-plus-river route planning, or Bhutan's SDF-aware Himalayan valley route can ship without touching core UI logic.
