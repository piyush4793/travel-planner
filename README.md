...
- **Panel-scoped Plan filters** — the Plan tab hosts a **Focus experiences** multi-select and a **Cities to visit** picker that only shape *this country's* offline itinerary (never any global/Calendar view). Each city is tagged with the country-level experiences it satisfies (authored, or derived from its itinerary content); selecting experiences boosts matching cities in auto-selection, surfaces matching cities first in the picker (with highlighted tags), and orders experience-matching activities first within each day. The **days slider** re-seeds its recommended value whenever the plan scope changes — travel style and budget (edited via the form) plus the panel-local experience/city focus (e.g. picking cities sums their recommended days; premium budgets nudge longer) — all clamped to the rule's max useful days
