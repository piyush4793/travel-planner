...
describe("tripPlans — P0", () => {
  describe("generateTripPlan", () => {
    it("generates a short trip plan", () => {
      const plan = generateTripPlan(COUNTRY_WITH_CITIES, [], 3);
    ...
    it("handles country with no cities", () => {
      const plan = generateTripPlan(COUNTRY_NO_CITIES, [], 7);
      expect(plan.days.length).toBeGreaterThan(0);
    });

    it("day labels follow 'Day N' format", () => {
      const plan = generateTripPlan(COUNTRY_WITH_CITIES, ["CityA"], 3);
    ...
    it("uses the generic fallback entry when no cities are selected and zero days are requested", () => {
      const plan = generateTripPlan(COUNTRY_NO_CITIES, [], 0);
    ...
    it("adds deeper exploration and final days entries for longer generic custom trips", () => {
      const twoWeeks = generateTripPlan(COUNTRY_NO_CITIES, [], 14);
      const threeWeeks = generateTripPlan(COUNTRY_NO_CITIES, [], 21);
    ...
    it("warns when custom city selections are tighter than the available days", () => {
      const plan = generateTripPlan(COUNTRY_WITH_CITIES, ["CityA", "CityB", "CityC"], 2);
  ...
  describe("generic fallback with selected cities", () => {
    it("uses city-based planning when specific cities are selected", () => {
      const plan = generateTripPlan(COUNTRY_WITH_CITIES, ["CityA", "CityB"], 30);
  ...
  it("formats generic plan cost ranges as rupee ranges", () => {
    const plan = generateTripPlan(COUNTRY_WITH_CITIES, [], 10);
  ...
  describe("custom style with selected cities", () => {
    it("returns one day-group per selected city", () => {
      const plan = generateTripPlan(COUNTRY_WITH_CITIES, ["CityA", "CityB"], 8);
    ...
    it("uses city-based activities that mention each selected city", () => {
      const plan = generateTripPlan(COUNTRY_WITH_CITIES, ["CityA", "CityB", "CityC"], 12);
  ...
  describe("empty-data resilience (India-style non-seed country)", () => {
    for (const days of [1, 3, 7, 21]) {
      it(`does not crash for days=${days} with no experiences/budget/cities`, () => {
        const plan = generateTripPlan(EMPTY_COUNTRY, [], days);
        expect(plan.days.length).toBeGreaterThan(0);
        expect(plan.costPerPerson).toContain("₹");
    ...
    it("tolerates selectedCities referencing an empty country", () => {
      expect(() =>
        generateTripPlan(EMPTY_COUNTRY, ["Nowhere"], 5),
      ).not.toThrow();
    });
  });
});
...
describe("tripPlans — rule-based DP selection", () => {
  ...
  it("fills exactly the requested days and keeps route order", () => {
    const plan = generateTripPlan(COUNTRY as never, [], 6, RULE as never);
  ...
  it("honours user-selected cities and only those", () => {
    const plan = generateTripPlan(COUNTRY as never, ["Alpha", "Gamma"], 5, RULE as never);
  ...
  it("expands and warns when the day budget can't fit all selected cities", () => {
    ...
    const plan = generateTripPlan(COUNTRY as never, ["Alpha", "Beta", "Gamma"], 2, RULE as never);
  ...
  it("plan cost tracks the selected basis and scales with trip length", () => {
    ...
    const couple = generateTripPlan(withBudget as never, [], 6, RULE as never, "couple");
    expect(couple.costPerPerson).toBe("₹2L – ₹4L");
    expect(couple.costBasis).toBe("couple");
    const solo = generateTripPlan(withBudget as never, [], 6, RULE as never, "solo");
    expect(solo.costPerPerson).toBe("₹1L – ₹2L");
    expect(solo.costBasis).toBe("solo");
    const family = generateTripPlan(withBudget as never, [], 6, RULE as never, "family4");
    ...
    const longer = generateTripPlan(withBudget as never, [], 12, RULE as never, "couple");
    expect(longer.costPerPerson).toBe("₹3L – ₹6L");
  });

  it("defaults to the couple basis when none is supplied", () => {
    const withBudget = { ...COUNTRY, budgetBreakdown: { solo: "₹1L–₹2L", couple: "₹2L–₹4L", family4: "₹4L–₹8L" } };
    const plan = generateTripPlan(withBudget as never, [], 6, RULE as never);
  ...
  it("surfaces experience-matching activities first within a day", () => {
    const base = generateTripPlan(EXP_COUNTRY as never, ["Foodville"], 2, EXP_RULE as never);
    expect(base.days[0].activities[0]).toContain("Old Fort");

    const focused = generateTripPlan(EXP_COUNTRY as never, ["Foodville"], 2, EXP_RULE as never, "couple", ["Street Food"]);
  ...
  it("boosts experience-matching cities in auto selection", () => {
    ...
    const auto = generateTripPlan(EXP_COUNTRY as never, [], 1, EXP_RULE as never);
    expect(auto.days.map((d) => d.label).join()).toContain("Dryton");

    const focused = generateTripPlan(EXP_COUNTRY as never, [], 1, EXP_RULE as never, "couple", ["Street Food"]);
...
describe("recommendedDaysForSelection", () => {
  ...
  it("uses the travel-style default when nothing is selected", () => {
    ...
    expect(
      recommendedDaysForSelection({ rule: RULE, recDays: 9, maxDays: 15, selectedCities: [], selectedExperiences: [] }),
    ).toBe(9);
  });

  it("sums the recommended days of explicitly picked cities", () => {
    expect(
      recommendedDaysForSelection({ rule: RULE, recDays: 9, maxDays: 15, selectedCities: ["Alpha", "Gamma"], selectedExperiences: [] }),
    ).toBe(7);
  });
  ...
  it("sums matching cities' days for a focus experience", () => {
    ...
    expect(
      recommendedDaysForSelection({ rule: RULE, recDays: 9, maxDays: 15, selectedCities: [], selectedExperiences: ["Street Food"] }),
    ).toBe(2);
  });
  ...
  it("ignores the budget nudge on a pristine (unscoped) panel", () => {
    ...
    expect(
      recommendedDaysForSelection({ rule: RULE, recDays: 9, maxDays: 15, selectedCities: [], selectedExperiences: [], budgetTier: "premium" }),
    ).toBe(9);
    expect(
      recommendedDaysForSelection({ rule: RULE, recDays: 9, maxDays: 15, selectedCities: [], selectedExperiences: [], budgetTier: "budget" }),
    ).toBe(9);
  });

  it("applies the budget-tier nudge once the plan is scoped and clamps to maxDays", () => {
    // Cities picked (3+4 = 7) → nudge applies.
    const premium = recommendedDaysForSelection({ rule: RULE, recDays: 9, maxDays: 15, selectedCities: ["Alpha", "Gamma"], selectedExperiences: [], budgetTier: "premium" });
    const budget = recommendedDaysForSelection({ rule: RULE, recDays: 9, maxDays: 15, selectedCities: ["Alpha", "Gamma"], selectedExperiences: [], budgetTier: "budget" });
    expect(premium).toBe(Math.round(7 * 1.15)); // 8
    expect(budget).toBe(Math.round(7 * 0.85)); // 6
    // Clamp: huge city-sum capped at maxDays.
    expect(
      recommendedDaysForSelection({ rule: RULE, recDays: 9, maxDays: 6, selectedCities: ["Alpha", "Beta", "Gamma"], selectedExperiences: [], budgetTier: "premium" }),
    ).toBe(6);
  });

  it("falls back to recDays with no rule", () => {
    // No travel-style scaling: base defaults to recDays.
    expect(
      recommendedDaysForSelection({ rule: null, recDays: 5, maxDays: 10, selectedCities: ["X"], selectedExperiences: [] }),
    ).toBe(5);
  });
})
...
describe("topExperienceCities", () => {
  ...
  it("caps an experience day estimate at the top two cities, not every match", () => {
    ...
    expect(
      recommendedDaysForSelection({ rule: MULTI_RULE, recDays: 6, maxDays: 20, selectedCities: [], selectedExperiences: ["Street Food"] }),
    ).toBe(5);
  });
  ...
  it("builds an experience-only plan from the top cities, excluding weaker matches", () => {
    ...
    const plan = generateTripPlan(country, [], 6, MULTI_RULE, "couple", ["Street Food"]);
    const labels = plan.days.map((d) => d.label).join();
...
describe("resolvePlannedCities — composable intent union", () => {
  ...
  it("keeps the day estimate in step with the resolved union", () => {
    ...
    expect(
      recommendedDaysForSelection({ rule: RULE, recDays: 10, maxDays: 20, selectedCities: ["Oslo"], selectedExperiences: ["Fjords"] }),
    ).toBe(expected);
  });
  ...
  it("builds an itinerary that visits both the picked city and the experience champion", () => {
    ...
    const plan = generateTripPlan(country, ["Oslo"], 4, RULE, "couple", ["Fjords"]);
    const labels = plan.days.map((d) => d.label).join();
