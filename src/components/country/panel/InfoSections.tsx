...
function ExternalLinkIcon() {
  return (
    <svg className="h-3 w-3 shrink-0 text-ink-3 transition-colors group-hover:text-brand-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  return (
    <div className="rounded-xl bg-surface-2/50 p-3.5">
      <button onClick={handleToggle} aria-expanded={open} aria-controls={contentId} className="group flex w-full items-center gap-2 text-left focus-ring rounded">
        ...
        <span className="flex-1 text-[11px] font-semibold text-ink-2">Learn about {countryName} 🌐</span>
      </button>
      <div id={contentId} role="region" aria-label={`Learn about ${countryName}`} className={`grid motion-safe:transition-[grid-template-rows,opacity,margin] motion-safe:duration-300 ease-out ${open ? "mt-3 grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
        <div className="overflow-hidden">
          <div className={`border-l pl-3 transition-colors duration-300 ${open ? "border-brand-200" : "border-transparent"}`} aria-live="polite" aria-busy={loading}>
            {loading && (
              <div className="flex items-center gap-2 py-2">
                <span className="inline-block h-3 w-3 animate-spin rounded-full border-2 border-brand-400 border-t-transparent" />
                <span className="text-[11px] text-brand-700">Loading info…</span>
              </div>
            )}
            {!loading && error && (
              <div className="flex items-center gap-2 py-2">
                <span className="text-[11px] text-ink-2">Failed to load info</span>
                <button onClick={handleRetry} className="text-[11px] font-semibold text-brand-700 hover:text-brand-800 focus-ring rounded px-1.5 py-0.5">
                  Retry
                </button>
              </div>
            )}
              ...
              <div className="space-y-3">
                ...
                {(countryInfo.capital || countryInfo.currency || countryInfo.language) && (
                  <div className="flex flex-wrap gap-2">
                    {countryInfo.capital && (
                      <span className="rounded-full bg-brand-50 border border-brand-200 px-2.5 py-1 text-[10px] font-semibold text-brand-700">
                        🏛️ {countryInfo.capital}
                      </span>
                    )}
                    {countryInfo.currency && (
                      <span className="rounded-full bg-accent-50 border border-accent-200 px-2.5 py-1 text-[10px] font-semibold text-accent-700">
                        💰 {countryInfo.currency}
                      </span>
                    )}
                ...
                <p className="text-xs leading-relaxed text-ink-2">{countryInfo.summary}</p>

                <a
                  href={`https://en.wikipedia.org/wiki/${encodeURIComponent(countryName.replace(/ /g, "_"))}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block text-[10px] font-semibold text-brand-700 hover:text-brand-800 transition-colors"
                >
            ...
            {!loading && !countryInfo && open && (
              <p className="text-[11px] text-ink-3 py-1">Could not load info. Check your internet connection.</p>
            )}
          </div>
        </div>
      </div>
        ...
        {links.map((link) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <span className="text-xs font-semibold text-brand-700 group-hover:text-brand-800">{link.label}</span>
              <p className="text-[10px] text-ink-3 leading-snug mt-0.5">{link.description}</p>
            </div>
