...
/** Emerald/ivory colour tokens for the selectable card, keyed by selection state. */
...
const THEME: SelectableTheme = {
  focusRing: "focus-ring-emerald",
  card: (selected, matchesFocus) =>
    selected
      ? "border-brand-500 bg-brand-50 text-ink-1 shadow-sm ring-1 ring-brand-500/30"
      : matchesFocus
        ? "border-brand-300 bg-brand-50/70 text-ink-1 shadow-sm hover:border-brand-400 hover:shadow"
        : "border-line bg-white/85 text-ink-1 shadow-[0_1px_2px_rgba(20,40,30,0.05)] hover:border-line-strong hover:shadow",
  title: (selected) => (selected ? "text-brand-900" : "text-ink-1"),
  star: () => "text-brand-600",
  check: (selected) =>
    selected
      ? "border-brand-600 bg-brand-600 text-white"
      : "border-line-strong text-transparent",
  best: () => "bg-brand-100/80 text-brand-800",
  worst: () => "bg-rose-50 text-rose-600",
  exp: (_selected, on) =>
    on ? "bg-brand-700 text-white" : "bg-surface-3 text-brand-800",
  notes: () => "text-ink-2",
};
...
function CityCardInner({ city, selectable, selected, onToggle, activeExperiences }: Props) {
  ...
  return (
    <div className="rounded-xl border border-line bg-white/80 px-3.5 py-2.5 shadow-sm shadow-brand-950/5">
      <div className="flex items-center justify-between gap-2">
        ...
        {bestMonths.length > 0 && (
          <div className="flex shrink-0 gap-1">
            {bestMonths.slice(0, 3).map((month) => (
              <span key={month} className="rounded bg-brand-100 px-1.5 py-0.5 text-[9px] font-bold text-brand-700">
                {month.slice(0, 3)}
              </span>
            ))}
          </div>
      ...
      {city.notes && (
        <p className="mt-1 text-[11px] leading-snug text-ink-2 line-clamp-3">{city.notes}</p>
      )}
    </div>
  );
}
