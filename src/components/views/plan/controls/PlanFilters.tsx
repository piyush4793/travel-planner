...
const SLIDERS = (
  <svg className="h-4 w-4 text-brand-700" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden="true">
...
export default function PlanFilters({ country, options, selected, onToggle, onClear }: Props) {
  ...
  const triggerInner = (
    <>
      ...
      {count > 0 && (
        <span className="rounded-full bg-brand-600 px-1.5 py-0.5 text-[10px] font-bold leading-none text-white">{count}</span>
      )}
      {CARET}
    </>
  );
  ...
  const triggerClass = `flex min-h-[36px] items-center gap-1.5 rounded-full border px-3.5 py-2 text-[13px] font-semibold transition-colors focus-ring-emerald ${
    count > 0
      ...
      : "border-line-strong bg-surface-1 text-ink-1 hover:border-line-strong"
  }`;
  ...
  const HeaderBand = ({ onClose }: { onClose?: () => void }) => (
    <div className="flex shrink-0 items-center gap-2.5 border-b border-brand-100 bg-gradient-to-b from-brand-50 to-white px-4 py-3">
      <span aria-hidden="true" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-surface-1 shadow-sm ring-1 ring-brand-100">{SLIDERS}</span>
      <h3 className="min-w-0 flex-1 font-display text-[15px] font-bold leading-tight text-brand-950">Filters · {country}</h3>
      {count > 0 && (
        <span className="shrink-0 rounded-full bg-brand-600 px-1.5 py-0.5 text-[10px] font-bold leading-none text-white">{count}</span>
      )}
  ...
  return (
    <section className="px-4 py-3.5">
      <div className="mb-2.5 flex items-center gap-1.5">
        <h4 className="text-[11px] font-bold uppercase tracking-[0.07em] text-brand-800">{label}</h4>
        {count > 0 && (
          <span className="rounded-full bg-brand-100 px-1.5 py-0.5 text-[10px] font-bold leading-none text-brand-800">{count}</span>
        )}
        {onClear && count > 0 && (
          <button
            type="button"
            ...
            className="focus-ring-emerald ml-auto -my-1 inline-flex min-h-[32px] items-center rounded-full px-2.5 text-[11px] font-semibold text-brand-700 transition-colors hover:bg-brand-50 hover:text-brand-800"
          >
            Clear
          </button>
        )}
    ...
    <>
      ...
      {open && createPortal(
        <div className="fixed inset-0 z-[99999] flex flex-col justify-end" role="dialog" aria-modal="true" aria-label={`Filters for ${country}`}>
          ...
          />
          <div className="relative flex max-h-[80vh] w-full flex-col overflow-hidden rounded-t-3xl border-t border-brand-100 bg-surface-1 shadow-2xl safe-bottom motion-safe:animate-[slideUp_0.2s_ease-out]">
            <SheetGrip />
            {renderHeader(close)}
