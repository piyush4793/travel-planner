...
export default function CityDetailModal({ decision: d, onToggle, onClose }: Props) {
  ...
  return (
    ...
    >
      {isMobile && <span aria-hidden="true" className="mx-auto mb-3 h-1 w-9 shrink-0 rounded-full bg-line-strong" />}

      <button
        type="button"
        onClick={onClose}
        aria-label="Close details"
        className="focus-ring-emerald absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full bg-surface-3 text-ink-2 transition-colors hover:bg-line"
      >
      ...
      <div className="pr-10">
        <h2 className="font-display text-2xl font-bold text-ink-1">{d.name}</h2>
        {d.signal && (
      ...
      {d.brief && <p className="mt-4 text-[13.5px] leading-relaxed text-ink-body">{d.brief}</p>}
