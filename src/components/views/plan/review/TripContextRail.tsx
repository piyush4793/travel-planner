...
type Props = {
  ...
  homeCountry: string;
  /** Trip scope — molds readiness + leg framing (visa/border vs domestic). */
  scope?: TripScope;
 ...
 */
function CountryBeforeYouGo({ country, homeCountry, showHeading, flagFor }: { country: Country; homeCountry: string; showHeading: boolean; flagFor: (name: string) => string }) {
    ...
    <div className="space-y-2.5">
      {showHeading && (
        <p className="flex items-center gap-1.5 text-[11px] font-bold text-ink-1">
          <span aria-hidden="true">{flagFor(country.name)}</span>
          {country.name}
        </p>
      )}
 ...
 */
function TripContextRailInner({ countries, composedPlan, perCountryCost, homeCountry, scope = "international", flagFor = getCountryFlag, notes, onSaveNotes }: Props) {
          ...
          {perCountryCost.map((row) => (
            <li key={row.name} className="flex items-center justify-between gap-2 text-[11px]">
              <span className="flex min-w-0 items-center gap-1.5 text-ink-2">
                <span aria-hidden="true">{flagFor(row.name)}</span>
          ...
          {countries.length > 1 && (
            <li className="flex items-center justify-between gap-2 text-[11px] italic text-ink-4">
              <span className="flex items-center gap-1.5">
                <span aria-hidden="true">✈</span> {scope === "domestic" ? "Inter-city legs" : "Inter-country legs"}
              </span>
            ...
            {seasonCountries.map((c) => (
              <div key={c.name}>
                <p className="mb-1 flex items-center gap-1.5 text-[11px] font-bold text-ink-1">
                  <span aria-hidden="true">{flagFor(c.name)}</span>
                  {c.name}
                </p>
      ...
      {countries.length > 0 && (
        <RailSection title="Before you go" hint="learn, visas, links" variant={v}>
          <div className={multi ? "space-y-4" : undefined}>
            {countries.map((c) => (
              <CountryBeforeYouGo key={c.name} country={c} homeCountry={homeCountry} showHeading={multi} flagFor={flagFor} />
            ))}
          </div>
        </RailSection>
      )}
