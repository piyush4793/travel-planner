...
export default function PlanReviewReveal({ open, onClose, routeName, days, cities, seconds }: Props) {
  ...
  return (
    <ModalShell
      ...
      backdropClassName="bg-emerald-950/50 backdrop-blur-sm"
      className="relative mx-4 w-full max-w-sm overflow-hidden rounded-3xl border border-emerald-100 bg-white text-center shadow-2xl focus:outline-none motion-safe:animate-[scaleIn_0.22s_ease-out]"
    >
      <div className="bg-gradient-to-b from-emerald-50 to-white px-6 pb-6 pt-8">
        <div
          aria-hidden="true"
          className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-white text-3xl shadow-sm ring-1 ring-emerald-100"
        >
          ✨
        </div>
        <h2 className="font-display text-xl font-bold tracking-tight text-emerald-950">Your trip is ready</h2>
        <p className="mx-auto mt-1.5 max-w-[16rem] text-[13px] leading-relaxed text-ink-body">
          ...
          <span className="font-semibold text-emerald-700">{routeName}</span>
        </p>

        {showSpeed && (
          <span className="mt-3 inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-[11px] font-bold text-emerald-800">
            <span aria-hidden="true">⚡</span> Planned in {seconds}s
          </span>
        )}

        <p className="mt-4 flex items-center justify-center gap-1.5 text-[12px] font-semibold text-emerald-700">
