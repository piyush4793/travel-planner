...
- **Itinerary actions** (`ItineraryToolbar`, rendered in `TripReviewCanvas`): a single **slim, low-emphasis action toolbar** (icon + tiny label ghost buttons, evenly distributed — matching the app's icon-control convention, not a wall of CTAs) with **📤 Share** lightly emphasised (emerald) as the suggested action, plus **🎬 Cinematic** (shown when the wizard is given a `mainMapRef` and the composed route has ≥2 mappable stops), **📄 PDF** (gated by `pdfExport`) and **✨ AI plan** (gated by `llmPlanning`), each control self-gated by capability/flag. Cinematic now works for **single- and multi-country** Plan routes (see the scope-agnostic cinematic route model below).
...
- `buildSingleCountryRoute(plan, country, rule, homeCountry, comboCountries?)` — convenience wrapper used by the Country Panel and the single-country wizard path.
