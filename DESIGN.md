...
- **Surfaces mold from the selection via the `DestinationSource` seam** (`src/core/trip/destinationSource.ts`): a scope-keyed port `{scope, unitNoun(Plural), popular(), resolveUnit(), comboRecommendations(), dayBounds(), experiencesFor(names)}`. `getDestinationSource(scope)` returns the registered source — only `international` today (`internationalSource.ts`); a future `domestic`/India scope implements the same port over cities and every wizard surface renders unchanged. Nothing in the surfaces assumes "country". Key molded surfaces:
