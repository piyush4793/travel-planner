...
- **Header actions**: the wizard header shows a **travel-style badge** (`STYLE_META[primaryStyle]` — 🏃/🔭/🌿) and a **Visited** toggle (`aria-pressed`), both bound to the active destination via the shared `PlanActions` bundle (`planActions.ts`). The redundant duration·cost chip is hidden on the Review step (the pane's summary bar already carries it).
...
- **Right-rail Notes** (`PlanNotesSection`): mirrors the Country Panel notes behaviour (400ms debounce + blur flush, 4,000-char cap, "✓ Saved" flash) in the luxury theme; re-seeds on destination change; only renders when `onSaveNotes` is wired (i.e. `App` passed `store.updateNotes`).
