...
| `multiCountryPlanning` | `false` | free | Multi-select destination picker + trip tray on `#plan` (up to `MAX_TRIP_COUNTRIES`); phased rollout |
...
- **Multi-country selection (flag `multiCountryPlanning`, off by default)**: with the flag on, `DestinationPicker` chips toggle into an ordered selection (capped at `MAX_TRIP_COUNTRIES` from `core/utils/multiCountry.ts`; pure `toggleTripSelection` helper) confirmed via a sticky **"Plan trip →"** tray; `onStart(countries)` seeds `PlanView`'s `selection` state (`picked = selection[0]`). With the flag off, a chip tap starts a single-country trip immediately (`onStart([c])`) — unchanged behaviour. The wizard currently plans the primary destination; cross-country composition is the next phase.
