...
- **Experience tags**: experience selection is **panel-local only** — the country panel's Plan tab owns a `selectedExperiences` state that shapes just that country's offline itinerary (city-score boost in auto-selection + matching-cities-first ordering in the picker + experience-first activity ordering per day). Each `CityEntry` carries an `experiences?: string[]` tag set — an authored override on the rule JSON when present, otherwise derived at enrichment time from the city's notes + itinerary content via the shared `src/core/utils/cityExperiences.ts` matcher (single source of truth, reused by both engine and panel UI). There is no global/app-level experience filter; Trips and Calendar never filter by experience
- **Recommended-days slider**: the Plan tab's day slider re-seeds from `recommendedDaysForSelection()` (`tripPlans.ts`) — a pure function of travel style (pacing), budget tier (a ±15% `BUDGET_DAY_FACTOR` nudge), and the panel-local city/experience focus (explicit cities → sum of their rec days; a focus experience → sum of matching cities' rec days), clamped to `[1, maxDays]`. It reacts whenever any of these change; manual drags persist until the next scope change. Panel-local only — never touches App/Calendar state
