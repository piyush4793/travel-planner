...
- **Landing picker ordering** (`DestinationPicker`): the **Jump back in** board shows implicit Recents in MRU order (recorded when a destination enters Basics via `recordPlanned`, capped by `MAX_RECENTS=24`). Fresh users have no recents, so the board falls back to **Popular destinations** sorted by `popularityScore` desc.
...
| `tp_schema_version` | Persisted-data schema version (current: 2; see Schema migrations) |

### Schema migrations

`src/core/migrations.ts` owns forward-compatible persistence upgrades:

- `SCHEMA_VERSION` is the current on-disk shape version (`2`).
...
- Pre-versioning stores (data present, no version key) are treated as the v1 baseline; the v2 migration deletes legacy country-level `tp_visited` and `tp_favorites` keys, then stamps the current version.
