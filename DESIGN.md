...
User edits stored as full objects in `tp_customs`. On load, customs override seed entries by name; `tp_deleted` tombstones removed seeds. Applies to countries — saved trips are independent snapshots keyed by route signature (see the Saved Trips section).
...
Default trip length starts from rule `recDays`. The day slider then feeds `planItinerary`.
...
|---|---|---|---|
| `paidFeatures` | `true` | system | Master gate for premium features |
...
- **Surfaces mold from the selection via the `DestinationSource` seam** (`src/core/trip/destinationSource.ts`): a scope-keyed port `{scope, unitNoun(Plural), popular(), resolveUnit(), comboRecommendations(), dayBounds(), experiencesFor(names), loadUnit(name), ruleStore}`. `getDestinationSource(scope)` returns the registered source — **`international` (`internationalSource.ts`, world countries) and `domestic` (`domesticIndiaSource.ts`, India states) are both live**; `TripScope = "international" | "domestic"` and `hasDomesticScope()` gates the landing scope toggle (`ScopeToggle`, shown only where a domestic dataset exists). Each source exposes its own lazy `ConsolidatedLoader` as `ruleStore`, so the primary stop's `usePlanBuilder → useCountryRule(name, source.ruleStore)` loads scope-correct rule JSON (domestic states glob `data/domestic/india/rules/*.json`). Flags are scope-aware via `unitFlag(name, scope, homeCountry)` (a domestic state has no ISO code, so it renders the home-country flag, e.g. 🇮🇳, instead of a 🌍 fallback); `PlanView` binds one `flagFor` callback threaded through every flag-rendering plan surface (default `getCountryFlag` keeps international unchanged). Nothing in the surfaces assumes "country". Key molded surfaces:
...
- **Mobile**: a **fixed bottom tab bar** (ivory `bg-[#fbf9f3]`, `border-[#e7e1d2]`) owns primary navigation (the 2 views, Plan · Trips, icon-over-label with an emerald-tinted active pill behind the icon, `safe-bottom`); the top strip shrinks to brand + a compact **Install/Share + Settings** cluster (no hamburger menu). The bottom bar sits in the flex column so content scrolls above it.
...
- **Combos**: "Combine with" pills come from Country rule data, not trip data.
...
Destination detail lives in the Plan wizard: Places cards and `CityDetailModal` cover city choice, while the Route Canvas `TripContextRail` hosts `MonthHeatmap`, watch-outs, stopover tips, notes, and lazy `InfoSections` (`LearnAboutSection`, `PlanningResourcesSection`, `UsefulLinksSection`).
...
`data/worldCatalog.json` remains a 197-country sovereign coordinate/region fallback used internally by catalog seed builders; it is not a top-level view. The manifest expands coverage to 198 itinerary-backed destinations and bakes `bestMonths`/`worstMonths` into every entry, enabling fully offline full-catalog month-fit ranking on the Plan landing without lazy rule loads. `ManifestEntry` and seed builders surface those windows onto `Country` objects. The **`inSeed`** flag identifies 5 richer seed entries (Japan, Thailand, Switzerland, France, Italy) for catalog/enrichment; fresh users start with an empty Recents board.
...
type Country = {
  ...
  notes?: string;
  diet?: { vegetarian: string; vegan: string; phrases: string[] };  // veg/vegan guidance (domestic India today), surfaced in the Insights rail + PDF when present
};
...
- Pre-versioning stores (data present, no version key) are treated as the v1 baseline; the v2 migration deletes legacy country-level `tp_visited` and `tp_favorites` keys; the v3 migration clears the legacy `tp_my_list` so the reused key starts as an honest empty Recents ledger. Then it stamps the current version.
