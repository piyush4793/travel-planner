...
- **Experience tags**: experience selection is **panel-local only** — the country panel's Plan tab owns a `selectedExperiences` state that shapes just that country's offline itinerary (anchors auto-selection on the top one or two matching cities via `topExperienceCities()`, boosts those cities' value in the DP, orders matching cities first in the picker, and orders experience-relevant activities first per day). Each `CityEntry` carries an `experiences?: string[]` tag set — an authored override on the rule JSON when present, otherwise derived at enrichment time from the city's notes + itinerary content via the shared `src/core/utils/cityExperiences.ts` matcher (single source of truth, reused by both engine and panel UI). There is no global/app-level experience filter; Trips and Calendar never filter by experience
